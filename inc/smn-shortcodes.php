...
function sumun_shortcode_botones( $atts ) {
	...
	if ( $taxonomy ) {
		...
		if ( $terms ) {
			...
			foreach( $terms as $term ) {
				...
				$term_color_name = get_term_meta( $term->term_id, 'term-color-name', true );
				if ( $term_color_name ) {
					$term_color_name = 'btn-' . $term_color_name;
				} else {
					$term_color_name = 'btn-primary';
				}
	
				$r .= '<a class="btn btn-block '. $term_color_name .'" href="'. get_term_link( $term ) .'">'.$term->name.'</a>';
...
add_shortcode( 'botones', 'sumun_shortcode_botones' );

function sumun_shortcode_recientes( $atts ) {
	...
	ob_start();
	get_template_part( 'global-templates/recent-carousel', '', array( 'post_type' => $post_type ) );
	return ob_get_clean();

}
add_shortcode( 'recientes', 'sumun_shortcode_recientes' );
